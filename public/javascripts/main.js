...
function dialogListItem(name, date, content) {
	...
	return (
		...
		(curUser == name
			? 'list-group-item-success text-left'
			: 'list-group-item-info text-right') +
		'">' +
	...
	$.ajax({
		...
		success: function (data) {
			$('#dialogModel').on('show.bs.modal', function () {
				...
				modal
					.find('.modal-title')
					.html('<i class="fa fa-users"></i>&nbsp;&nbsp;漂流瓶对话')

				content = dialogListItem(
					data.msg.message[0][0],
					data.msg.message[0][4],
					data.msg.message[0][3]
				)
				for (var i = 1; i < data.msg.message.length; i++) {
					content += dialogListItem(
						data.msg.message[i][0],
						data.msg.message[i][3],
						data.msg.message[i][2]
					)
				}
				...
				footer =
					...
					'id="replyBtn" data-dismiss="modal" name="' +
					data.msg._id +
					'" ' +
...
function replyBottle(obj) {
	...
	$.ajax({
		...
		dataType: 'json',
		success: function (data) {
			console.log('回复的数据：', data)
			promptModalHandle(true, '回复瓶子成功!')
		},
		error: function () {
		...
		success: function (data) {
			$('#resultModal').on('show.bs.modal', function () {
				...
				footer =
					...
					'<button type="button" class="btn btn-primary" ' +
					'id="deleteBtn" data-dismiss="modal" name="' +
					data.msg._id +
					'" ' +
						...
						if (listArr.length) {
							for (var i = 0; i < listArr.length; i++) {
								content +=
									...
									'</p>' +
									'<a class="label label-info" onclick=\'bottleInfo(this)\' data-dismiss="modal" rel="' +
									listArr[i]._id +
									'">详情</a>' +
									'</li>'
							}
							...
							content =
								'<li class="list-group-item"><h3>暂无数据呈现……</h3></li>'
						}
